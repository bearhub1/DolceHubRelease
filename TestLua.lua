...
ContainerFrame.Position = UDim2.new(0, 185, 0, 40)
ContainerFrame.Size = UDim2.new(1, -185, 1, -40)
...
TabContainer.Size = UDim2.new(0, 185, 1, -40)
...
IconFrameCorner.CornerRadius = UDim.new(0,20)
...
IconIamge.Image = "https://www.roblox.com/headshot-thumbnail/image?userId="..game.Players.LocalPlayer.UserId.."&width=420&height=420&format=png"
...
function in_section:NewToggle(Name, Setting, Callback)
    ...
    ToggleButton.MouseButton1Click:Connect(function()
        default = not default 
        local imagetran = default and 0 or 1 
        local imagesize = default and UDim2.new(1,0,1,0) or UDim2.new(0,0,0,0)
...
local Default = Setting.Value or Setting.value or Setting.DefaultValue or Setting.defaultvalue or Setting.Default or Setting.default
...
if Default then
    SliderValue.Text = Default..Donvi
    ValueLabel.Text = Default..Donvi
    pcall(function()
        Callback(Default)
    end)
    Bar.Size = UDim2.new(1-((Max - Default)/(Max - Min)), 0, 0, 6)
end
...
function in_section:NewDropdown(Name,Setting,Callback)
    ...
    local DefaultValue = Setting.Value or Setting.Value or Setting.DefaultValue or Setting.Default or 'nil'
    ...
    if SearchSetting then
        ...
        if string.len(DefaultValue) > 17 then 
            Dropdownbox.Text = ''
            Dropdownbox.PlaceholderText = string.match(DefaultValue,'.................').."..."
        else 
            Dropdownbox.Text = ''
            Dropdownbox.PlaceholderText = DefaultValue
        end
        ...
        if string.len(DefaultValue) > 17 then 
            Dropdownbutton.Text = string.match(DefaultValue,'.................').."..."
        else 
            Dropdownbutton.Text = DefaultValue
        end 
